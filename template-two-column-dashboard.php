<?php
/*
  Template Name: Two Column Dashboard Page
 */
?>
<?php
	...
	$field_type = get_field('assessment_type_title');
?>
...
<?php echo $isLastAssessmentFormPage ? 'data-assessment-complete="' . do_shortcode('[user-answered-all-mandatory assessment="' . get_field("assessment_type_title") . '" min="1" max="10"]') . '"' : '';?>
						    	...
						    	<?php 
						    		...
						    		if(have_rows('breadcrumb_step_collection')):
										while (have_rows('breadcrumb_step_collection')):
                                            ...
                                            $slug=get_post_field( 'post_title', get_post());
                                            $form_name=stripos($slug,'post') !== false ? 'post-assessment-form-page-'.$stepNumber : 'assessment-form-page-'.$stepNumber;
											//$stepStatus = do_shortcode('[pixafy-unanswered-questions form_name="'.$form_name.'"]');
											$section_ready = do_shortcode('[pixafy-unanswered-mandatory form_name="'.$form_name.'" page_num="page-'.$stepNumber.'" slug="'.$current_slug.'"]');

											//echo '<div style="display:none;" data-form-name="'.$form_name.'" data-step="'.$stepNumber.'"><pre>';
											//var_dump($section_ready);
											//echo '</pre></div>';
								?>
						    		<li class="pix-form_breadcrumb_step<?php echo $isActive; echo $isLastChild; ?>" data-form-step="<?php echo $stepNumber; ?>">
						    			<span data-status="<?php echo $section_ready; ?>" class="pix-step_status fa <?php echo $section_ready ? 'fa-check' : 'fa-close' ; ?>"></span>
												...
												?>
														<li class="pix-article_item pix-dashboard_campus_resource_document_wrapper">
															<div class="pix-article_item_wrapper">
																<a onclick="ga('send', 'event', 'Resource Documents', 'click', '<?php echo str_replace("'", "", the_sub_field('cr_document_name')); ?>');"
																   target="_blank" href="<?php echo (the_sub_field('cr_document_url') ? the_sub_field('cr_document_url') : wp_get_attachment_url(the_sub_field('cr_document'))); ?>"><img src="<?php echo the_sub_field('cr_document_image'); ?>" alt="" title="" />
																</a>
																<a onclick="ga('send', 'event', 'Resource Documents', 'click', '<?php echo str_replace("'", "", the_sub_field('cr_document_name')); ?>');"
																   target="_blank" href="<?php echo (the_sub_field('cr_document_url') ? the_sub_field('cr_document_url') : wp_get_attachment_url(the_sub_field('cr_document'))); ?>">
